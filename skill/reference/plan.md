...
**Output**: A design brief that can be handed off to {{command_prefix}}fk craft, or directly to {{command_prefix}}fk for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to {{command_prefix}}fk, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use {{command_prefix}}fk craft instead, which runs this command internally.)
