...
```bash
node .trae/skills/fk/scripts/live.mjs
```
...
```
LOOP:
  node .trae/skills/fk/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node .trae/skills/fk/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .trae/skills/fk/scripts/live-status.mjs
node .trae/skills/fk/scripts/live-resume.mjs --id SESSION_ID
node .trae/skills/fk/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .trae/skills/fk/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .trae/skills/fk/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .trae/skills/fk/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .trae/skills/fk/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .trae/skills/fk/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node .trae/skills/fk/scripts/live-server.mjs stop
```
...
```bash
node .trae/skills/fk/scripts/detect-csp.mjs
```
