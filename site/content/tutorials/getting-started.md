...
order: 1
description: "Install Impeccable, run /fk init once to establish design context, and run /fk polish on something that already exists. The fastest path to seeing what Impeccable changes about AI-generated design."
---
...
For example: `/fk polish the pricing page`, or `/fk audit the checkout`. Type `/fk` alone to see the full list.

If you use a command often, pin it with `/fk pin <command>` to create a standalone shortcut (for example, `/fk pin audit` gives you `/audit` directly).
...
/fk init
/fk polish the page you care about
```
...
This auto-detects your AI coding tool and writes the right skill files for it (for example, `.claude/skills/` or `.cursor/skills/`). It works with Cursor, Claude Code, GitHub Copilot, Gemini CLI, Codex CLI, and every other major harness. Reload your tool and type `/`. You should see `/fk` in the autocomplete. Type it and the argument hint will show the available commands.

Prefer a different setup? Claude Code users can install the plugin with `/plugin marketplace add pbakaus/fk`, and the general-purpose `npx skills add pbakaus/fk` still works (though it installs one shared build for all harnesses rather than the one compiled for yours).

When a new version ships later, run `npx fk-skills update` from the same project root. `npx fk-skills check` tells you first whether you are behind, and plugin users update from the `/plugin` menu instead.
...
This is the most important step. Design without context produces generic output. The `/fk init` command runs a short setup interview and writes `PRODUCT.md` at the root of your project.
...
At the end of `/fk init`, the skill offers to run `/fk document` for you. Say yes. It looks at your existing colors, type, components, and tokens, then writes `DESIGN.md` in the [Google Stitch DESIGN.md format](https://stitch.withgoogle.com/docs/design-md/format/).
...
- `/fk critique the landing page` runs a full design review with scoring, persona tests, and automated detection. It is the best way to find what to fix next.
- `/fk audit the checkout` runs accessibility, performance, theming, responsive, and anti-pattern checks against the implementation. Useful before shipping.
- `npx fk-skills detect src/` runs the deterministic detector directly from the terminal. See [Detector CLI](/docs/detector).
- `/fk craft a pricing page for enterprise customers` runs the full shape-then-build flow on a brand new feature.
- **Pin your favorites.** If you reach for one command constantly, `/fk pin audit` makes `/audit` work as a standalone shortcut without reversing the consolidation.
- `/fk redo this hero section` works too. Any description after `/fk` applies the design principles to the task.

## Common issues

- **The skill says "no design context found"**. You skipped step 2. Run `/fk init` first.
