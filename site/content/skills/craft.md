...
`/fk craft` is the end-to-end build command. Give it a feature description and it runs the whole pipeline: structured discovery, reference loading, implementation, visual iteration. Use it when you are starting a new feature from zero and want the whole workflow in one invocation.
...
If you only want the thinking without the code, use `/fk shape` standalone. If you already have a clear vision and just want to build, call `/fk` directly with your feature description. `craft` sits in between: structured, complete, opinionated.

## How it works

`craft` runs four phases in order:

1. **Shape the design.** Runs `/fk shape` internally: a short discovery conversation about purpose, users, content, constraints, and goals. The output is a design brief you can read and push back on.
...
```
/fk craft a pricing page for a developer tool
```
...
- **Using it for small changes.** `craft` is for new features, not touch-ups. For existing code, reach for `/fk polish`, `/fk critique`, or a specific refinement command instead.
