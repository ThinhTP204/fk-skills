...
`/fk` is the main command. Use it in two ways:

- Run `/fk` by itself when you want the skill to inspect the project and recommend what to do next.
- Add a plain-English request when you know the outcome but not the exact command.

Reach for `/fk` directly when:
...
If this is a new project, start with `/fk init`. That creates the setup files every other command reads.

## How it works

Most AI-generated UIs fail the same way: generic fonts, purple gradients, card grids on card grids, glassmorphism everywhere. `/fk` gives the model stronger design instructions before it writes code.
...
On first use in a project, `/fk` may route you into `init`: a short interview that writes `PRODUCT.md` and offers to write `DESIGN.md`. Future commands read those files without asking again.
...
```
/fk build me a pricing page for a developer tool
```

Both prompts are vague on purpose. `/fk` will choose the right command or run the work directly, using your setup files when they exist.
...
v3.0 consolidated 18 standalone skills into a single `/fk` with 23 commands. If you miss the short form of a command, pin it back:

```
/fk pin critique
```

From now on, `/critique` invokes `/fk critique` directly. It writes a lightweight redirect skill that delegates to the parent, so updates to the skill flow through without re-pinning.

Useful pins to try:

- `/fk pin polish` for final-pass work
- `/fk pin audit` for deterministic a11y/perf checks
- `/fk pin live` for the browser iteration flow
- `/fk pin critique` for design review

To remove: `/fk unpin critique`. Pins live as directories named after the command in your harness skills folder (`.claude/skills/critique/`, `.cursor/skills/critique/`, etc.), so you can also delete them manually.
...
- **Expecting it to fix existing code.** `/fk` is for creation. For refinement, reach for `/fk polish`, `/fk distill`, or `/fk critique` instead.
- **Running it before `init` has saved context.** On a fresh project it will interview you mid-flight, which is fine but slower. Running `/fk init` first is smoother.
- **Picking the wrong brand/product lane.** Marketing pages and app screens need different defaults. If `PRODUCT.md` has no `## Register` field (legacy), run `/fk init` to add it.
