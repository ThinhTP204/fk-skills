 ...
 *
 * `pin check` creates a lightweight /check skill that redirects to /fk check.
 * `unpin check` removes that shortcut.
 *
...
const VALID_COMMANDS = [
  'build', 'setup', 'tokens', 'spec', 'plan',
  'review', 'check',
  'finish', 'amplify', 'calm', 'trim', 'prod', 'welcome', 'live',
  'motion', 'color', 'type', 'space', 'joy', 'wow',
  'copy', 'responsive', 'perf',
];

// Marker to identify pinned skills (so unpin doesn't delete user skills)
const PIN_MARKER = '<!-- fk-pinned-skill -->';
...
/**
 * Find harness skill directories that have a fk skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have fk installed
    const fkDir = join(skillsDir, 'fk');
    if (existsSync(fkDir) || existsSync(join(skillsDir, 'i-fk'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /fk ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}fk ${command}\`.

Invoke {{command_prefix}}fk ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (created > 0) {
    ...
    console.log(`You can now use /${command} directly (delegates to /fk ${command}).`);
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /fk ${command} to access it.`);
  } else {
